<form method="POST" action="{{ $action_url }}" autocomplete="off" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" id="exampleFormControlInput1" placeholder="" name="name"  value="{{ isset($brand) ? $brand->name : ''}}">
        @error('name')
        ...
        <div class="border rounded">
            <div id="avatarWrapper" class="border-bottom d-flex" style="display: none;">
                @if (isset($brand->avatar))
                    <img src="{{ asset('storage/' . $brand->avatar) }}" alt="" class="m-3" width="120px" height="120px">    
                @endif                    
            </div>
            <input type="file"  class="form-control" id="avatar" placeholder="" name="avatar" value="" onchange="previewAvatar(event)">
        </div>
        @error('avatar')
